...
void solve()
{
    ...
    int res = *std::min_element(f[n], f[n] + idx);
    if (res >= INF) res = -1;
    printf("Case %d: %d\n", ++TC, res);
}
...
记忆化搜索：https://www.acwing.com/solution/content/66855/

数位DP搜索树/迭代写法：https://www.acwing.com/solution/content/31142/

参数法和贡献法：https://leetcode.cn/problems/number-of-digit-one/solutions/1750339/by-endlesscheng-h9ua

本题没有必要写贡献法，因为和迭代法比较类似。
...
int dfs(int i, int cnt, bool isLimit)
{
    if (i < 0) return cnt == k;
    if (cnt > k) return 0;
    if (!isLimit && f[i][cnt] >= 0) return f[i][cnt];

    int up = isLimit ? ds[i] : b - 1;
    up = std::min(up, 1);  // 本题的特殊之处
    int res = 0;
    for (int d = 0; d <= up; d++)
    {
        res += dfs(i - 1, cnt + d, isLimit && d == ds[i]);
    }
    
    if (!isLimit) f[i][cnt] = res;
    return res;
}
...
int dfs(int i, int pre, bool isLimit)
{
    if (i < 0) return 1;
    if (!isLimit && f[i][pre] >= 0) return f[i][pre];

    int up = isLimit ? ds[i] : 9;
    int res = 0;
    for (int d = pre; d <= up; d++)
    {
        res += dfs(i - 1, d, isLimit && d == ds[i]);
    }
...
https://www.acwing.com/solution/content/287465/
...
int f[N][N];  // f(i, j)表示有i位的并且最高位为j的windy数的个数

void init()
{
    // f(1, 0)也必须为1，所有的f(i, 0)仅为递推使用
    for (int j = 0; j <= 9; j++) f[1][j] = 1;  
    for (int i = 2; i <= 10; i++)  // 2e9有10位
        for (int j = 0; j <= 9; j++)
            for (int k = 0; k <= 9; k++)
                if (std::abs(k - j) >= 2)
                    f[i][j] += f[i - 1][k];
...
{
    std::vector<int> ds;
    for (int i = n; i; i /= 10) ds.push_back(i % 10);
    ...
    for (int i = high; i >= 0; i--)
    {
        ...
        for (int j = (i == high); j < d; j++)
            if (std::abs(j - pre) >= 2)
                res += f[i + 1][j];

        if (std::abs(d - pre) < 2) break;
        pre = d;
        if (!i) res++;
    }
...
int dfs(int i, int pre, bool isStart, bool isLimit)
{
    if (i < 0) return !isStart;
    if (!isLimit && f[isStart][i][pre] >= 0) return f[isStart][i][pre];

    int up = isLimit ? ds[i] : 9;
    int res = 0;

    for (int d = 0; d <= up; d++)
        if (isStart || std::abs(d - pre) >= 2)
            res += dfs(i - 1, d, isStart && d == 0, isLimit && d == ds[i]);

    if (!isLimit) f[isStart][i][pre] = res;
    return res;
}
...
int dp(int n)
{
    ...
    for (int i = n; i; i /= 10) ds.push_back(i % 10);
    memset(f, -1, sizeof f);
    return dfs(ds.size() - 1, 0, true, true);
}
...
https://www.acwing.com/solution/content/15556/

https://www.acwing.com/solution/content/287483/
...
int dp(int n)
{
    if (n == 0) return 1;  // 如果不加这一行n=0时会返回0

    ds.clear();
    for (int i = n; i; i /= 10) ds.push_back(i % 10);
    int res = 0;
    for (int i = ds.size() - 1, sum = 0; i >= 0; i--)
    {
        int d = ds[i];
        for (int j = 0; j < d; j++)
        {
...
int dfs(int i, int sum, bool isLimit)
{
    if (i < 0) return sum == 0;
    if (!isLimit && f[i][sum] >= 0) return f[i][sum];

    int up = isLimit ? ds[i] : 9;
    int res = 0;
    for (int d = 0; d <= up; d++)
    {
        res += dfs(i - 1, (d + sum) % p, isLimit && d == ds[i]);
    }

    if (!isLimit) f[i][sum] = res;
    return res;
}
...
https://www.acwing.com/solution/content/15582/

https://www.acwing.com/solution/content/287542/
...
{
    f[0][0] = 1;
    f[1][1] = f[1][2] = 1, f[1][0] = 7;
    for (int i = 2; i <= 10; i++)
    {
        int sum = f[i - 1][0] + f[i - 1][1] + f[i - 1][2];
        for (int d = 0; d <= 9; d++)
            if (d != 4)
            {
                if (d == 2) f[i][1] += sum;
                else if (d == 6) f[i][2] += f[i - 1][0] + f[i - 1][2];
                else f[i][0] += sum;
            }
    }
}
...
int dfs(int i, bool isSix, bool isLimit)
{
    if (i < 0) return 1;
    if (!isLimit && f[i][isSix] >= 0) return f[i][isSix];

    int up = isLimit ? ds[i] : 9;
    int res = 0;
    for (int d = 0; d <= up; d++)
        if (d != 4 && (d != 2 || !isSix))
            res += dfs(i - 1, d == 6, isLimit && d == ds[i]);
    
    if (!isLimit) f[i][isSix] = res;
    return res;
}
...
int dp(int n)
{
    ...
    for (int i = n; i; i /= 10) ds.push_back(i % 10);
    memset(f, -1, sizeof f);
    return dfs(ds.size() - 1, false, true);
}
...
https://www.acwing.com/solution/content/47501/
...
const int N = 20, M = 10, P = int(1e9) + 7;

struct Node
{
    int a, b, c;  // 个数，数字和，数字平方和
...
auto init = []
    {
        ...
        for (int i = 1; i < N; i++)
        {
            ...
            powP[i] = LL(powP[i - 1]) * 10 % P;
        }

        for (int j = 0; j <= 9; j++)
            if (j != 7)
            {
                f[1][j][j % 7][j % 7] += Node(1, j, j * j);
            }
        for (int i = 2; i <= 18; i++)
            for (int j = 0; j <= 9; j++)
                if (j != 7)
                    for (int a = 0; a < 7; a++)
                        for (int b = 0; b < 7; b++)
                        {
                            ...
                            for (int k = 0; k <= 9; k++)
                                if (k != 7)
                                {
                                    cur += f[i - 1][k][mod(a - j * pow7[i - 1], 7)][mod(b - j, 7)];
                                }
                            int t = LL(j) * powP[i - 1] % P;  // j * 10^{i - 1}
                            cur.c = (LL(cur.a) * t % P * t % P + 2LL * t * cur.b % P + cur.c) % P;
                            cur.b = (LL(cur.a) * t % P + cur.b) % P;
                        }
        return 0;
    }();
...
int dp(LL n)
{
    ...
    for (int i = ds.size() - 1; i >= 0; i--)
    {
        ...
        int a = mod(-accA * pow7[i + 1], 7), b = mod(-accB, 7);
        for (int j = 0; j < d; j++)
            if (j != 7)
            {
                Node cur = get(i + 1, j, a, b);
                int t = accA % P * powP[i + 1] % P;
                res = (res + 
                    LL(cur.a) * t % P * t % P + 
                    2LL * t * cur.b % P + 
                    cur.c) % P;
...
const int N = 25, M = 10, MOD = int(1e9) + 7;
...
{
    pow[0] = 1;
    for (int i = 1; i < N; i++) pow[i] = LL(pow[i - 1]) * 10 % MOD;
}

Node dfs(int i, int sum, int num, bool isLimit)
{
    if (i < 0)
    {
    ...
    if (!isLimit && f[i][sum][num].a >= 0) return f[i][sum][num];
    ...
    for (int d = 0; d <= up; d++)
        if (d != 7)
        {
            Node t = dfs(i - 1, (sum + d) % 7,
                (num * 10 + d) % 7, isLimit && d == ds[i]);
            int x = LL(d) * pow[i] % MOD;  // d * 10 ^ i
            t.c = (t.c + LL(x) * x % MOD * t.a % MOD) % MOD;
            t.c = (t.c + LL(x) * 2 * t.b % MOD) % MOD;
            t.b = (t.b + LL(x) * t.a % MOD) % MOD;
            res += t;
        }

    if (!isLimit) f[i][sum][num] = res;
    return res;
}
...
int dp(LL n)
{
    ...
    for (; n; n /= 10) ds.push_back(n % 10);
    memset(f, -1, sizeof f);
    return dfs(ds.size() - 1, 0, 0, true).c;
}
