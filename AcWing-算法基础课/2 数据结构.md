...
这里采用数组的方式来存储单链表。并且可以存储多个链表，在[哈希表的拉链法](#拉链法)中会介绍。

**模板：**

```c++
// idx表示新插入节点的下标
// ne[0]存储了第一个节点的位置，即指针0存储的位置是dummy节点
// 空指针也为0，遍历链表时作为终止条件
int e[N], ne[N], idx = 1; 
...
[双链表](https://www.acwing.com/problem/content/description/829/)
...
const int N = 1e5 + 10;
const int head = 0, tail = N - 1;

int m;
int e[N], ne[N], pe[N], idx = 1;

void init() // 有头尾两个dummy节点，这种方式没法做循环双链表
{
    ne[head] = tail, pe[tail] = 0;
}

void insert_k( int k, int x )
{
    e[idx] = x, ne[idx] = ne[k], ne[k] = idx;
    pe[idx] = k, pe[ne[idx]] = idx;
    idx++; // 不能和上一行的最后一句合在一起写成pe[ne[idx]] = idx++;
}

void delete_k( int k )
{
    ne[pe[k]] = ne[k], pe[ne[k]] = pe[k];
}
...
int main()
{
    ...
    while ( m-- )
    {
        ...
        {
            int k, x; scanf( "%d%d", &k, &x );
            if ( op[1] == 'L' ) insert_k( pe[k], x );
            else insert_k( k, x );
        }
        else
        {
            int x; scanf( "%d", &x );
            if ( op[0] == 'L' ) insert_k( head, x );
            else if ( op[0] == 'R' ) insert_k( pe[tail], x );
            else delete_k( x );
        }
    }
    
    for ( int p = ne[head]; p != tail; p = ne[p] ) printf( "%d ", e[p] );
