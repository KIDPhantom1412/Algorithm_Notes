...
int main()
{
    ...
    {
        int a, b, w; scanf("%d%d%d", &a, &b, &w);
        G[a][b] = G[b][a] = max(G[a][b], double(100 - w) / 100);
    }
    scanf("%d%d", &A, &B);
...
int bfs()
{
    ...
    while (!q.empty())
    {
        ...
        for (int p = H[cur.v]; p; p = L[p].n)
        {
            ...
            if (!st[v])
            {
                if (v == n) return cur.s + 1;
                q.push(Node(v, cur.s + 1));
            }
        }
    }
...
int main()
{
    ...
    for (int i = 1; i <= n; i++)
    {
        int price, cnt; scanf("%d%d%d", &price, &level[i], &cnt);
        W[0][i] = min(W[0][i], price);
        while (cnt--)
        {
            int id; scanf("%d%d", &id, &price);
            W[id][i] = min(W[id][i], price);
        }
    }
